...
class ModelMetaclass(type):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        # Definde default SELECT, INSERT, UPDATE and DELETE command:
...
class Model(dict, metaclass=ModelMetaclass):
    ...
    @classmethod
    async def nextId(cls, where=None, args=None):
        # find next_id by select and where
        sql = ['select Max(id) from `%s`' % cls.__table__]
        if where:
            sql.append('where')
            sql.append(where)
